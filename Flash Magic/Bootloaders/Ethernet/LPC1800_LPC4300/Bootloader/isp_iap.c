...
int decode_length(char *inbuf, int *skipnum)
{
	int lencharnum;
	int lenchar;
	int len = 0;
	...
	if(inbuf[0] == 0x60)
	{
    // is this a high-performance record?
		if ((inbuf[1] == 0x20) && (inbuf[2] == 0x60) && (inbuf[3] == 0x20))
		{
			lencharnum = inbuf[4] - SP;
			for (lenchar = 0; lenchar < lencharnum; lenchar++)
			{
				len += inbuf[5 + lenchar] - SP;
			}
			
			// skip preamble
			*skipnum = 5 + lencharnum - 1;
			
			return len;
		}
		// not a high-performance record, no data
		else
		{
			*skipnum = 0;
			return(0x00);
...
unsigned uudecode(char * inbuf, char * outbuf, int * count)
{
    ...
    sum=0;
    out_byte_count=0;
    in_byte_count = decode_length(inbuf, &skipnum);
	  inbuf += skipnum;
...
void execute_user_code(void)
{
    ...
    user_code_entry = (void (*)(void))(*((unsigned long *)(BANK_A_USER_START_SECTOR_ADDRESS + 4)) | 0x00000001);
    user_code_entry();
}
